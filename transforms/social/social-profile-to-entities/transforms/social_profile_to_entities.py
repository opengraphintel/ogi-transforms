...
USER_AGENT = "OGI Social Profile Entity Extractor/1.0"
...
class SocialProfileToEntities(BaseTransform):
    ...
    def _extract_entities(self, *, source_entity: Entity, source_url: str, html: str, max_results: int, same_host_links_only: bool) -> tuple[list[Entity], list[Edge], list[str]]:
        ...
        username_from_url = PROFILE_PATH_RE.match(parsed_source.path or "")
        if username_from_url:
            self._append_entity(entities, edges, seen, source_entity=source_entity, entity_type=EntityType.USERNAME, value=username_from_url.group(1), label="observed username", source_name=self.name, properties={"observed_on_url": source_url, "observation_type": "profile_url"})
